...
const RestaurantCard = props => {
    ...
    <StyledContainer onClick={props.onClick}>
         <div className="text restaurant-info">
            <h1 style={{'display': props.detail === true ? 'none' : 'inherit'}} className="text restaurant-name">{props.name}</h1>
            <p style={{'display': props.detail === true ? 'none' : 'inherit'}} className="text restaurant-cat">{props.category}</p>
        </div>
